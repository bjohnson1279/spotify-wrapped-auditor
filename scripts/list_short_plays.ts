...
const runList = () => {
    ...
    console.log("Song | MS | End | Start | Shuffle | Date");
    console.log("------------------------------------------");
    ...
    events.forEach((e: any) => {
        ...
        if (UNDERCOUNTED.includes(name) && e.ms_played >= 30000 && e.ms_played < 60000) {
            console.log(`${name} | ${e.ms_played} | ${e.reason_end} | ${e.reason_start} | ${e.shuffle} | ${e.ts}`);
        }
    });
};
